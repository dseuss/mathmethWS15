...
\begin{solution}
  ...
  \[
    5 \mat{a} = 5 \colvec{ 3 \\ 1 } = \colvec{ 5 \cdot 3 \\ 5 \cdot 1 } = \colvec{ 15 \\ 5 }
  \]
  Nacheinanderausführung von Multiplikation und Addition (\quotes{Punkt- vor Strichrechnung}), ebenfalls komponentenweise
  \[
    3 \mat{a} + (-2) \mat{b} = \colvec{ 3 \cdot 3 \\ 3 \cdot 1 } + \colvec{ -2 \cdot (-1) \\ -2 \cdot 2 }
    = \colvec{ 9 \\ 3 } + \colvec{ 2 \\ -4 } = \colvec{ 11 \\ -1 }
  \]

  \item Stelle $\mat{a}$ als Linearkombination $x\cdot \vec{e}_1 + y\cdot \vec{e}_2$ dar, wobei die Koeffizienten $x,y \in \RR$ zunächst unbestimmt sind
  \[
